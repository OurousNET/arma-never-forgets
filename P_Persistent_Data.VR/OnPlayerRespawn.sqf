...
_FirstSpawn = _oldunit getVariable ["FIRSTSPAWN", true];

if !(_FirstSpawn) then {
	hint "NOT FIRST SPAWN!!!!";
	...
	_newunit setVariable ["FIRSTSPAWN", false];
	sleep 600;
	deleteVehicle _oldUnit;
	}
else {
	hint "FIRST SPAWN, DID YOUR GEAR AND LOCATION LOAD CORRECTLY!!!!";
	waitUntil {!isNull player}
	call Perseus_fnc_Client_SetPlayer_Attributes;
	_newunit setVariable ["FIRSTSPAWN", false];
	};
